...
type HelloServiceInterface interface {
	SayHello(req string, res *string) error
}
...
// SayHello 实现接口并且绑定方法
func (h *HelloServer) SayHello(req string, res *string) error {
	*res = req + "你好！"
	return nil
}

func SimpleRpcServerV2() {
...
func SimpleRpcServerV1() {
	// 1. 创建rpc服务
	err := rpc.RegisterName("hello", &HelloServer{})
	if err != nil {
		fmt.Println("gorpc.RegisterName error:", err)
		return
	}
	//2.设置监听端口
	listener, err := net.Listen("tcp", "127.0.0.1:1234")
